    ...
    (PostgRESTQueryBuilder.prototype )[operation ] = new Proxy(
      (PostgRESTQueryBuilder.prototype )[operation ],
      {
        apply(target, thisArg, argumentsList) {
          ...
          debugBuild.DEBUG_BUILD && logger.debug.log(`Instrumenting ${operation} operation's PostgRESTFilterBuilder`);
...
const instrumentSupabaseClient = (supabaseClient) => {
  if (!supabaseClient) {
    debugBuild.DEBUG_BUILD && logger.debug.warn('Supabase integration was not installed because no Supabase client was provided.');
    return;
  }
