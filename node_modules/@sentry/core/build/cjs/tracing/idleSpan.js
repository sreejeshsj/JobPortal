...
function startIdleSpan(startSpanOptions, options = {}) {
  ...
  function onIdleSpanEnded(endTimestamp) {
    ...
    logger.debug.log(`[Tracing] Idle span "${spanJSON.op}" finished`);
    ...
    childSpans.forEach(childSpan => {
      ...
      if (childSpan.isRecording()) {
        ...
        debugBuild.DEBUG_BUILD &&
          logger.debug.log('[Tracing] Cancelling span since span ended early', JSON.stringify(childSpan, undefined, 2));
      }
      ...
      if (debugBuild.DEBUG_BUILD) {
        ...
        if (!spanStartedBeforeIdleSpanEnd) {
          logger.debug.log('[Tracing] Discarding span since it happened after idle span was finished', stringifiedSpan);
        } else if (!spanEndedBeforeFinalTimeout) {
          logger.debug.log('[Tracing] Discarding span since it finished after idle span final timeout', stringifiedSpan);
        }
      }
...
function _startIdleSpan(options) {
  ...
  debugBuild.DEBUG_BUILD && logger.debug.log('[Tracing] Started span is an idle span');
