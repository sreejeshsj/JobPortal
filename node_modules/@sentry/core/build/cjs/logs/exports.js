...
function _INTERNAL_captureLog(
  ...
  if (!client$1) {
    debugBuild.DEBUG_BUILD && logger.debug.warn('No client available to capture log.');
    return;
  }
  ...
  if (!enableLogs) {
    debugBuild.DEBUG_BUILD && logger.debug.warn('logging option not enabled, log will not be captured.');
    return;
  }
  ...
  const log = beforeSendLog ? logger.consoleSandbox(() => beforeSendLog(processedLog)) : processedLog;
  if (!log) {
    client$1.recordDroppedEvent('before_send', 'log_item', 1);
    debugBuild.DEBUG_BUILD && logger.debug.warn('beforeSendLog returned null, log will not be captured.');
    return;
  }
